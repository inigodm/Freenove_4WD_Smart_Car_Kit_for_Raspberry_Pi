...
class Zurixeren_Kotxea:
    from motor import Ordinary_Car
    PWM = Ordinary_Car()

    def aurrera(self, urratsak):
        move = urratsak * 100
        self.PWM.set_motor_model(urratsak, urratsak, urratsak, urratsak)  # Forward

    def atzera(self, urratsak):
        move = urratsak * -100
        self.PWM.set_motor_model(urratsak, urratsak, urratsak, urratsak)  # Forward

    def eskuinera(self, urratsak):
        move = urratsak * -100
        self.PWM.set_motor_model(urratsak, -urratsak, urratsak, -urratsak)  # Forward

    def ezkerrera(self, urratsak):
        move = urratsak * -100
        self.PWM.set_motor_model(-urratsak, urratsak, -urratsak, urratsak)  # Forward

    def bira(self, urratsak):
        move = urratsak * -100
        self.PWM.set_motor_model(-urratsak, -urratsak, urratsak, urratsak)  # Forward

    def biraBesteAldera(self, urratsak):
        move = urratsak * 100
        self.PWM.set_motor_model(-urratsak, -urratsak, urratsak, urratsak)  # Forward

kotxea = Zurixeren_Kotxea()
